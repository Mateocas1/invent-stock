/**
 * PostgreSQL Consumption Repository
 *
 * Implementation of ConsumptionRepository using PostgreSQL.
 */
...
export class PostgreSQLConsumptionRepository implements ConsumptionRepository {
  ...
  async findByProductId(productId: string, days: number): Promise<ConsumptionHistory[]> {
    const rows = await this.db.query<ConsumptionRow>(
      `SELECT * FROM consumption_history
       WHERE product_id = $1 AND date >= CURRENT_DATE - INTERVAL '${days} days'
       ORDER BY date DESC`,
      [productId],
    );
