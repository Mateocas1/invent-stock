/**
 * PostgreSQL Stock Alert Repository
 *
 * Implementation of StockAlertRepository using PostgreSQL.
 */
...
export class PostgreSQLStockAlertRepository implements StockAlertRepository {
  ...
  async findByProductId(productId: string): Promise<StockAlert[]> {
    const rows = await this.db.query<AlertRow>(
      'SELECT * FROM stock_alerts WHERE product_id = $1 ORDER BY created_at DESC',
      [productId],
    );
  ...
  async acknowledge(alertId: string): Promise<void> {
    await this.db.execute('UPDATE stock_alerts SET acknowledged = true WHERE id = $1', [alertId]);
  }

  async deleteOldAcknowledged(olderThanDays: number): Promise<void> {
    await this.db.execute(
      `DELETE FROM stock_alerts
       WHERE acknowledged = true AND created_at < NOW() - INTERVAL '${olderThanDays} days'`,
    );
  }
}
