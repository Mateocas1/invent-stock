/**
 * PostgreSQL Recipe Repository
 *
 * Implementation of RecipeRepository using PostgreSQL.
 */
...
export class PostgreSQLRecipeRepository implements RecipeRepository {
  ...
  async findByServiceAndProduct(serviceId: string, productId: string): Promise<Recipe | null> {
    const row = await this.db.queryOne<RecipeRow>(
      'SELECT * FROM recipes WHERE service_id = $1 AND product_id = $2',
      [serviceId, productId],
    );
  ...
  async deleteByServiceId(serviceId: string): Promise<void> {
    await this.db.execute('DELETE FROM recipes WHERE service_id = $1', [serviceId]);
  }

  async findByProductId(productId: string): Promise<Recipe[]> {
    const rows = await this.db.query<RecipeRow>('SELECT * FROM recipes WHERE product_id = $1', [
      productId,
    ]);
