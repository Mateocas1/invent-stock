 ...
 * SQL is embedded to avoid file copy issues during build
 */

import { DatabaseAdapter } from './adapters/DatabaseAdapter';

/**
 * Embedded bootstrap SQL for Railway PostgreSQL
 * This creates all tables and seeds initial data in ONE transaction
 */
const BOOTSTRAP_SQL = `
...
INSERT INTO _migrations_failsafe (id, name) VALUES
  ('failsafe_bootstrap', 'Initial bootstrap from FailsafeMigrator')
...
export class FailsafeMigrator {
  ...
  /**
   * Execute embedded bootstrap SQL to create schema + seed
   */
  static async runBootstrap(db: DatabaseAdapter): Promise<void> {
    try {
      console.log('🔧 FAILSAFE: Executing bootstrap SQL...');
      ...
      for (const statement of statements) {
        try {
          ...
          if (errorMsg.includes('already exists') || errorMsg.includes('duplicate key')) {
            ...
            console.warn(`⚠️ FAILSAFE: Query warning: ${statement.substring(0, 60)}...`);
          }
        }
      }

      console.log(
        `✅ FAILSAFE: Bootstrap completed (${successCount} statements, ${skipCount} skipped)`,
      );
    } catch (error) {
